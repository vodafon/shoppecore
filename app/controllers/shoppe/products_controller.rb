module Shoppe
  class ProductsController < Shoppe::ApplicationController
    ...
    def index
      @products = Shoppe::Product.root.includes(:stock_level_adjustments, :product_category, :variants).order("reviews_count DESC").group_by(&:product_category).sort_by { |cat,pro| cat.name }
    end
